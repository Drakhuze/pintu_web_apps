...
const TableTokenRow = ({ row }: Props) => {
  ...
  const isChangePrice = Number(prevPrice) === Number(row.price) ? false : true;
  const isChangeLowPrice = Number(prevLowPrice) === Number(row.lowPrice) ? false : true;
  const isChangeHighPrice = Number(prevHighPrice) === Number(row.highPrice) ? false : true;
